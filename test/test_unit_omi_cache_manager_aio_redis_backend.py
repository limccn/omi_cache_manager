...
def get_cache():
    return aioredis_cache_use_pool


@pytest.mark.asyncio
async def test_backend_default_with_pool(event_loop):
    val = await aioredis_cache_use_pool.get("foo")
    assert val is None


@pytest.mark.asyncio
async def test_backend_default_no_pool(event_loop):
    val = await aioredis_cache.get("foo")
    assert val is None
...
if __name__ == '__main__':
    pytest.main(['test_unit_omi_cache_manager_aio_redis_backend.py'])
