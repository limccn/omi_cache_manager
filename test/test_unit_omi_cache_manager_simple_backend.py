...
simple_cache_from_instance = AsyncCacheManager(
    None,
    cache_backend=SimpleCacheBackend(config={"CACHE_KEY_PREFIX": "SOME_PREFIX:"})
)
...
def test_backend_implement(setup_module):
    simple_cache = AsyncCacheManager(
        None,
        cache_backend=SimpleCacheBackend(config={})
    )


def test_backend_implement_error(setup_module):
    try:
        foo_cache = AsyncCacheManager(
            None,
...
if __name__ == '__main__':
    pytest.main(['test_unit_omi_cache_manager_simple_backend.py'])
