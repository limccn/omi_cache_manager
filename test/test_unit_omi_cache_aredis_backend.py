...
aredis_cache_backend = ARedisBackend(
    config={
        ...
        "CACHE_KEY_PREFIX": "A_REDIS_BACKEND_UNIT_TEST:"
    }
)

aredis_cache_backend_with_pool = ARedisBackend(
    config={
        "CACHE_REDIS_SCHEME": "redis",
        "CACHE_REDIS_HOST": "192.168.201.169",
        "CACHE_REDIS_PORT": 6379,
        ...
        "CACHE_KEY_PREFIX": "A_REDIS_BACKEND_UNIT_TEST_POOL:"
    }
)
...
def get_cache():
    return aredis_cache_backend_with_pool


@pytest.mark.asyncio
async def test_backend_default_with_pool(event_loop):
    val = await aredis_cache_backend_with_pool.get("foo")
    assert val is None


@pytest.mark.asyncio
async def test_backend_default_no_pool(event_loop):
    val = await aredis_cache_backend.get("foo")
    assert val is None
...
async def test_backend_exec_single(event_loop):
    ...
    assert val is "1"
    val = await get_cache().execute("INCR", "A_REDIS_BACKEND_UNIT_TEST_POOL:foobar")
    assert val is 2
    val = await get_cache().execute("GET", "foobar")
    assert val is "2"
    val = await get_cache().execute("INCRBY", "A_REDIS_BACKEND_UNIT_TEST_POOL:foobar", 2)
    assert val is 4
    val = await get_cache().execute("GET", "foobar")
    assert val is "4"
